...
/***************----- 端口数据缓冲区 -----***************/
unsigned char TRx_Buffer[128];	//端口接收数据缓冲区 
...
{
	SPI_Send_Byte((unsigned char)(dt >> 8));	//写数据高位
...
* 说明    : 无
*******************************************************************************/
void Write_W5500_nByte(unsigned short reg, unsigned char *dat_ptr, unsigned short len)
{
	unsigned short i;
...
* 说明    : 无
*******************************************************************************/
void Write_SOCK_Data_Buffer(SOCKET s, unsigned char *dat_ptr, unsigned short len)
{
	unsigned short offset,offset1;
	unsigned short i;
...
void Socket_Init(SOCKET s)
{
	...
	switch(s)
	{
		case 0:
			//设置端口0的端口号
			Write_W5500_SOCK_2Byte(0, Sn_PORT, ((unsigned short)S0_Port[0] << 8) | S0_Port[1]);	
...
unsigned char Socket_Connect(SOCKET s)
{
	...
	if(Read_W5500_SOCK_1Byte(s,Sn_SR)!=SOCK_INIT)//如果socket打开失败
	{
		...
		return 0x00;//FALSE;//返回FALSE(0x00)
	}
	Write_W5500_SOCK_1Byte(s,Sn_CR,CONNECT);//设置Socket为Connect模式
	return 0xff;//TRUE;//返回TRUE,设置成功
}
